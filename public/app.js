...
async function localApi(path, options = {}) {
  ...
  if (method === "GET" && url.pathname === "/api/runs/next-queued") {
    return cloneJson(runs.find((run) => ["queued_for_worker", "queued_for_codex", "waiting_for_worker"].includes(run.automation?.status) || (!run.automation && run.steps?.inputs?.status === "done")) || null);
  }
